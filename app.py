...
FREE_TRIAL_LIMIT = 3
WA_URL = ‘https://wa.me/573228246703?text=Hola%2C+quiero+acceso+completo+a+Saber+AI+Santander.+Adjunto+comprobante.’

SYSTEM_PROMPT = (
’Eres el tutor lider de Saber AI Santander, preparacion ICFES 2026, Santander Colombia. ’
’Reglas: 1) Identifica la competencia evaluada. ’
’2) Nunca des la respuesta directa, haz primero una pregunta orientadora. ’
’3) Tono de docente santandereano: directo, alentador, profesional. ’
’4) Si insisten, da una pista adicional, no la solucion. ’
‘5) Celebra avances con frases motivadoras de Bucaramanga.’
)

st.set_page_config(page_title=‘Saber AI Santander’, page_icon=‘🎓’, layout=‘wide’)

st.markdown(
‘<style>’
‘.stApp{background:#f0f4f8}’
‘.hdr{background:linear-gradient(135deg,#1a3c6e,#2d6a9f,#f5a623);color:white;’
‘padding:1.2rem 2rem;border-radius:12px;margin-bottom:1.5rem}’
‘.hdr h1{margin:0;font-size:1.8rem;font-weight:800}’
‘.hdr p{margin:.2rem 0 0;opacity:.9;font-size:.95rem}’
‘.cu{background:#1a3c6e;color:white;border-radius:18px 18px 4px 18px;’
‘padding:.75rem 1.1rem;margin:.4rem 0;max-width:75%;float:right;clear:both}’
...
‘.cf{clear:both}’
‘.tbadge{background:#fff3cd;border:1px solid #f5a623;border-radius:8px;’
‘padding:.4rem .9rem;font-size:.85rem;color:#856404;font-weight:600;’
‘display:inline-block;margin-bottom:.8rem}’
‘.pw{background:linear-gradient(135deg,#1a3c6e,#2d6a9f);color:white;’
‘border-radius:16px;padding:2rem;text-align:center;margin-top:1rem}’
‘.pw h2{font-size:1.5rem;margin-bottom:.5rem}’
‘.pw p{opacity:.92;font-size:1rem;margin-bottom:1rem}’
‘.pi{background:rgba(255,255,255,.15);border-radius:10px;padding:1rem;margin:1rem 0}’
‘.pi strong{color:#f5a623;font-size:1.1rem}’
‘.wa{display:inline-block;background:#25d366;color:white !important;’
‘padding:.7rem 1.8rem;border-radius:30px;text-decoration:none !important;font-weight:700}’
‘.ok{background:#d4edda;color:#155724;border-radius:8px;padding:.6rem 1rem;’
‘font-weight:600;margin-top:.5rem}’
‘.er{background:#f8d7da;color:#721c24;border-radius:8px;padding:.6rem 1rem;’
‘font-weight:600;margin-top:.5rem}’
‘</style>’,
...
st.markdown(
‘<div class="ok">Acceso completo activo<br>’
‘<small>Codigo: <b>’ + str(st.session_state.active_code) + ‘</b></small></div>’,
unsafe_allow_html=True,
)
st.markdown(’—’)
...
css = ‘ok’ if st.session_state.code_ok else ‘er’
st.markdown(’<div class="' + css + '">’ + st.session_state.code_msg + ‘</div>’, unsafe_allow_html=True)
st.markdown(’—’)
...
st.markdown(
‘<div class="hdr"><h1>Saber AI Santander</h1>’
‘<p>Tu tutor inteligente para el ICFES 2026 - Basado en los cuadernillos oficiales</p></div>’,
unsafe_allow_html=True,
)
...
‘<a href="' + WA_URL + '" target="_blank" class="wa">’
...
label = ‘Estudiante’ if m[‘role’] == ‘user’ else ‘Tutor’
lines.append(label + ‘: ’ + m[‘content’])
history = ‘\n’.join(lines)
prompt = (
‘Eres el tutor lider de Saber AI Santander. ’
‘Analiza el historial y genera un Plan de Estudio de 7 dias para el ICFES 2026.\n\n’
‘HISTORIAL:\n’ + history + ‘\n\n’
‘INSTRUCCIONES:\n’
‘- Identifica areas debiles del estudiante.\n’
‘- Plan Dia 1 a Dia 7: tema, competencia ICFES, ejercicios, objetivo, consejo santandereano.\n’
‘- Resumen final de fortalezas y areas de mejora.\n’
‘- Tono calido y motivador de docente de Santander.\n’
‘- ICFES es el 26 de julio de 2026.’
)
...
‘Fecha ICFES: 26 de julio de 2026\n’
‘==========================================\n\n’
+ plan +
‘\n\n==========================================\n’
